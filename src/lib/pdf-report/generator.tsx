...
export const collectReportData = async (
  ...
  // Calculate statistics (case-insensitive severity comparison)
  const totalComponents = nodes.length;
  const totalConnections = edges.length;
  const criticalIssues = reportFindings.filter(f => f.severity.toLowerCase() === 'critical').length;
  const highIssues = reportFindings.filter(f => f.severity.toLowerCase() === 'high').length;
  const mediumIssues = reportFindings.filter(f => f.severity.toLowerCase() === 'medium').length;
  const lowIssues = reportFindings.filter(f => f.severity.toLowerCase() === 'low').length;
